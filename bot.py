...
if DATABASE_URL:
    try:
        ...
        # محاولة إنشاء الفهارس لتحسين الأداء (اختياري)
        try:
...
async def get_seasons_stats(series_id):
    ...
    try:
        ...
        print(f"❌ خطأ في جلب إحصائيات المواسم: {e}")
        return []
...
# ==============================
# 4. معالجة تفاصيل المحتوى مع التقسيم إلى صفحات (محسّن)
# ==============================
async def show_content_details(update: Update, context: ContextTypes.DEFAULT_TYPE, content_id, page=1):
    ...
    try:
        ...
        # ============================================
            ...
            # جلب جميع الأجزاء (الأفلام قد تكون لها أجزاء متعددة)
            ...
            if not episodes:
                ...
                # تجميع الأجزاء حسب رقم الجزء (الموسم)
                seasons = {}
                for ep in episodes:
...
# ==============================
# 6. الدالة الرئيسية مع تحسينات الاتصال
# ==============================
def main():
